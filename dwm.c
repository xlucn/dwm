...
#define WIDTH(X)                ((X)->w + 2 * (X)->bw + (((X)->isfloating || !selmon->lt[selmon->sellt]->arrange) ? 0 : gappx))
#define HEIGHT(X)               ((X)->h + 2 * (X)->bw + (((X)->isfloating || !selmon->lt[selmon->sellt]->arrange) ? 0 : gappx))
...
static void updatewinarea(Monitor *m);
...
static int tpad;             /* top padding for text */
	...
	*lastbutton = '0' + ev->button;
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > (x = selmon->mw - (int)TEXTW(stext) - getsystraywidth() + lrpad)) {
			click = ClkStatusText;
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		while (1) {
			...
			ctmp = *ts;
			*ts = '\0';
			drw_text(drw, m->mw - tw - stw + tx, 0, TEXTW(tp) - lrpad, bh, 0, tpad, tp, 0);
			tx += TEXTW(tp) - lrpad;
			if (ctmp == '\0')
				break;
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, w, bh, lrpad / 2, tpad, occ & 1 << i ?
		         (m == selmon && selmon->sel && selmon->sel->tags & 1 << i ?
		          tags_curr[i]: tags_busy[i]) : tags[i], urg & 1 << i);
		x += w;
	}
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeNorm]);
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, tpad, m->ltsymbol, 0);

	if ((w = m->mw - tw - stw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeSel : SchemeNorm]);
			drw_text(drw, x, 0, w, bh, lrpad / 2, tpad, m->sel->name, 0);
			if (m->sel->isfloating)
	...
	drw_map(drw, m->barwin, 0, 0, m->mw - stw, bh);
}
...
{
	static Monitor *prevmon = NULL;
	Monitor *m;
	XCrossingEvent *ev = &e->xcrossing;

	if ((ev->mode != NotifyNormal || ev->detail == NotifyInferior) && ev->window != root)
		return;
	if ((m = recttomon(ev->x_root, ev->y_root, 1, 1)) != prevmon && prevmon) {
		unfocus(selmon->sel, 1);
		selmon = m;
		focus(NULL);
	}
...
manage(Window w, XWindowAttributes *wa)
{
	...
	updatesizehints(c);
	updatewmhints(c);
	if (c->isfloating) {  /* center window if floating */
...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MotionNotify:
			...
			if (abs(selmon->wx - gappx / 2 - nx) < snap)
				nx = selmon->wx - gappx / 2;
			else if (abs((selmon->wx + selmon->ww) + gappx / 2 - (nx + WIDTH(c))) < snap)
				nx = selmon->wx + selmon->ww + gappx / 2 - WIDTH(c);
			if (abs(selmon->wy - gappx / 2 - ny) < snap)
				ny = selmon->wy - gappx / 2;
			else if (abs((selmon->wy + selmon->wh) + gappx / 2 - (ny + HEIGHT(c))) < snap)
				ny = selmon->wy + selmon->wh + gappx / 2 - HEIGHT(c);
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	noborder = (!c->isfullscreen && !c->isfloating &&
	            &monocle == c->mon->lt[c->mon->sellt]->arrange);

	if (!(c->isfloating || !selmon->lt[selmon->sellt]->arrange)) {
		x -= (noborder ? 1 : -1) * gappx / 2;
		y -= (noborder ? 1 : -1) * gappx / 2;
		w += (noborder ? 1 : -1) * gappx;
		h += (noborder ? 1 : -1) * gappx;
	}
	c->oldx = c->x; c->x = wc.x = x;
	c->oldy = c->y; c->y = wc.y = y;
	c->oldw = c->w; c->w = wc.width = w + noborder * c->bw * 2;
	c->oldh = c->h; c->h = wc.height = h + noborder * c->bw * 2;
	wc.border_width = (noborder ^ 1) * c->bw;
...
resizemouse(const Arg *arg)
{
	...
	ocx = c->x;
	ocy = c->y;
	ocx2 = c->x + c->w + 2 * c->bw - 1;
	ocy2 = c->y + c->h + 2 * c->bw - 1;
	if (!XQueryPointer(dpy, c->win, &dummy, &dummy, &di, &di, &nx, &ny, &dui))
		return;
	horizcorner = nx < c->w / 2;
	vertcorner = ny < c->h / 2;
	CurResize = vertcorner ? (horizcorner ? CurResizeTL : CurResizeTR)
	                       : (horizcorner ? CurResizeBL : CurResizeBR);
	if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
	...
	do {
		...
		case MotionNotify:
			...
			nw = MAX((horizcorner ? ocx2 - ev.xmotion.x : ev.xmotion.x - ocx) - 2 * c->bw + 1, 1);
			nh = MAX((vertcorner ? ocy2 - ev.xmotion.y : ev.xmotion.y - ocy) - 2 * c->bw + 1, 1);
...
{
	gappx += MAX(-gappx, arg->i);
	updatebarpos(selmon);
	updatewinarea(selmon);
	arrange(selmon);
}
...
setup(void)
{
	...
	lrpad = drw->fonts->h + barpaddingh * 2;
	tpad = barpaddingtop;
	bh = drw->fonts->h + barpaddingtop + barpaddingbottom;
	if (barheightmin > bh) {
		tpad += (barheightmin - bh) / 2;
		bh = barheightmin;
	}
	updategeom();
	/* init atoms */
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		if (showsystray && m == systraytomon(m))
			w -= getsystraywidth();
		m->barwin = XCreateWindow(dpy, root, m->mx, m->by, w, bh, 0, DefaultDepth(dpy, screen),
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		i->y = MAX(0, (bh - systrayiconsize) / 2);
